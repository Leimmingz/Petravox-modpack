...
[![Launcher](https://img.shields.io/badge/Launcher-v3.4.0-16b981?style=flat-square)](https://github.com/Leimmingz/Petravox-modpack/releases/latest)
...
Petravox est un serveur Minecraft moddé privé basé sur le modpack **Fantasy Vs. Science**, enrichi de 216 mods. Il tourne sur Forge 1.20.1 et est accessible via un launcher dédié qui gère automatiquement l'installation et les mises à jour.
...
### v2.8.13 — 21 juillet 2026 ← actuelle
- Fix auto-updater : suppression du flag `/T` sur `taskkill` — il tuait l'arbre de processus entier, y compris le script bat lui-même (enfant du launcher), empêchant le redémarrage après téléchargement d'une mise à jour

### v2.8.12 — 21 juillet 2026
- Fix auth : token Minecraft rafraîchi automatiquement au démarrage via `refresh_token` — corrige la déconnexion en jeu après avoir fermé/rouvert le launcher (le launcher affichait "connecté" avec un token expiré)
- Fix auth : session proprement invalidée si le refresh échoue, au lieu de garder un compte "connecté" avec un token mort

### v2.8.11 — 21 juillet 2026
...
- Fix auto-updater : `taskkill` + timeout 3s avant remplacement (fix `python312.dll` verrouillé)
...
- Fix réinitialiser paramètres : remet le thème sombre

### v2.8.4 — 13 juillet 2026
- Fix changement de thème : fenêtre cachée pendant le rebuild (plus de flash clair/sombre)
- Fix mods récents conservés après changement de thème (config en cache, pas de re-fetch)
- Fix stats de jeu : valeurs alignées sous leurs labels

### v2.8.3 — 13 juillet 2026
- Fix auto-updater : `start /D` dans le bat pour garantir le bon répertoire au relancement (fix erreur DLL _tkinter)
